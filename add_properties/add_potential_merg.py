...
pmdist_f_ar = df['dist_f_ar'].values
    # print(pmpos_ar)
    # print(pdfm['pos_f_ar'].values)
pmpos_list = df['pos_f_ar'].values
pmvel_list = df['vel_f_ar'].values
...
pmpos_f_ar = np.stack(np.array(pmpos_list))
pmvel_f_ar = np.stack(np.array(pmvel_list))

pmpos_if_ar = np.stack(np.array(df['pos_if_ar'].values))
pmvel_if_ar = np.stack(np.array(df['vel_if_ar'].values))
...
def get_potential(ix):
    snap = snap_if_ar[ix]
    pot_f = get_potential_snapshot(pmdist_f_ar[ix], 99)
    pot_if = get_potential_snapshot(pmdist_if_ar[ix], snap)
    return pot_if, pot_f
