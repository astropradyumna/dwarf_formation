...
def get_vpeak(ix):
    ...
    # Vpeak in this case is simply the maximum Vmax of the subhalo
    return np.max(sub['SubhaloVmax'])


mpeak_ar = Parallel(n_jobs=5, pre_dispatch='1.5*n_jobs')(delayed(get_vpeak)(ix) for ix in tqdm(range(int(len(snap_if_ar)))))

df['vpeak_ar'] = mpeak_ar
