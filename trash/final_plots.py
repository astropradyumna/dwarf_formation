...
def plot_radial_density_dist(fof_no = 0):
    '''
    This plot is for the radial density distribution of the subhalos to compare with the DM density distribution and stelar density distribution
    '''
    ...
    # Nstar_ar = Nstar_ar * (cNm_ar[-1] + cNs_ar[-1]) #Normalizing the particles thing to cutoff model total temporarily

    #before we change any of the arrays, lets calculate number in each bin
    volume_bin = np.append(rpl[0]**3, rpl[1:]**3 - rpl[:-1]**3)*4*np.pi/3
    volume_bin2 = np.append(rpl2[0]**3, rpl2[1:]**3 - rpl2[:-1]**3)*4*np.pi/3
    Ndm_bin = np.append(Ndm_ar[0], Ndm_ar[1:] - Ndm_ar[:-1])/volume_bin
    Nstar_bin = np.append(Mstar_ar[0], Mstar_ar[1:] - Mstar_ar[:-1])/volume_bin
    Ntng_bin = np.append(Ntng_ar[0], Ntng_ar[1:] - Ntng_ar[:-1])/volume_bin2
    pNm_bin = np.append(pNm_ar[0], pNm_ar[1:] - pNm_ar[:-1])/volume_bin2
    pNs_bin = np.append(pNs_ar[0], pNs_ar[1:] - pNs_ar[:-1])/volume_bin2
    cNm_bin = np.append(cNm_ar[0], cNm_ar[1:] - cNm_ar[:-1])/volume_bin2
    cNs_bin = np.append(cNs_ar[0], cNs_ar[1:] - cNs_ar[:-1])/volume_bin2

    

    norm_part = 1/(Ndm_bin[-1]+Nstar_bin[-1])
    Ndm_bin = Ndm_bin * norm_part
    Nstar_bin = Nstar_bin * norm_part
    Nstar_and_dm = (Ndm_bin + Nstar_bin)
    Ntng_bin = Ntng_bin/Ntng_bin[-1]
    pN_bin = (pNs_bin + pNm_bin)/(pNs_bin[-1] + pNm_bin[-1])
    cN_bin = (cNs_bin + cNm_bin)/(cNs_bin[-1] + cNm_bin[-1])

    fig, ax = plt.subplots(figsize = (6, 6))
    ax.plot(rpl2, Ntng_bin, color = 'blue', label = r'TNG')
    ax.plot(rpl2, pN_bin, color = 'red', label = r'power law ($>10\,\rm{M_\odot}$)')
    ax.plot(rpl2, cN_bin, color = 'darkgreen', label = r'cutoff ($>10\,\rm{M_\odot}$)')

    ax.plot(rpl, Ndm_bin, color = 'black', ls = '--', label = 'DM in TNG', alpha = 0.5)
    ax.plot(rpl, Nstar_bin, color = 'black', ls = ':', label = 'Stars in TNG', alpha = 0.5)
    ax.plot(rpl, Nstar_and_dm, color = 'black', ls = '-', label = 'Stars and DM in TNG', alpha = 0.5)
    ax.legend(fontsize = 8)
    ax.set_xlabel('Distance from center (kpc)')
    ax.set_ylabel(r'Number density in each bin $[\rm{kpc^{-3}}]$')
    ax.set_xscale('log')
    ax.set_yscale('log')
    plt.tight_layout()
    plt.savefig(this_fof_plotppath + 'cumulative_rad_dist_perbin.png')
    ...
    ax.plot(rpl2, Ntng_ar, color = 'blue', label = r'TNG')
    ax.plot(rpl2, pN_ar, color = 'red', label = r'power law ($>10\,\rm{M_\odot}$)')
    ax.plot(rpl2, cN_ar, color = 'darkgreen', label = r'cutoff ($>10\,\rm{M_\odot}$)')
    # ax.plot(rpl, N_all_ar, color = 'purple', ls = '--', lw = 0.5, label = r'Model (all)')
    ax.plot(rpl, Ndm_ar, color = 'black', ls = '--', label = 'DM in TNG', alpha = 0.5)
    ax.plot(rpl, Mstar_ar, color = 'black', ls = ':', label = 'Stars in TNG', alpha = 0.5)
    ax.plot(rpl, Nstar_and_dm, color = 'black', ls = '-', label = 'Stars and DM in TNG', alpha = 0.5)
    ax.legend(fontsize = 8)
    ax.set_xlabel('Distance from center (kpc)')
    ax.set_ylabel(r'Number density $(<r)$ $[\rm{kpc^{-3}}]$')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_ylim(bottom = 0.75, top = 5e4)
    ...
    def get_dens_rh(mvir):
        ...
        this_nfw = NFWProfile(Mvir = mvir, cvir = conc, z = 0)
    ...
    mstar_1e11, dens_rh_1e11 = get_dens_rh(1e11)
    mstar_1e10, dens_rh_1e10 = get_dens_rh(1e10)
    mstar_1e9, dens_rh_1e9 = get_dens_rh(1e9)
    mstar_1e8, dens_rh_1e8 = get_dens_rh(1e8)
    mstar_1e6, dens_rh_1e6 = get_dens_rh(1e6)
    mstar_1e4, dens_rh_1e4 = get_dens_rh(1e4)


    ax2.plot(rpl, rho_dm, color = 'black', ls = '--', label = 'DM in TNG', alpha = 0.5)
    ax2.plot(rpl, rho_star, color = 'black', ls = ':', label = 'Stars in TNG', alpha = 0.5)
    ax2.plot(rpl, rho_dm + rho_star, color = 'black', ls = '-', label = 'Stars and DM in TNG', alpha = 0.5)
    ax2.axhline(dens_rh_1e11, color = 'blue', label = r'$\log M_{\rm{star}} = $' + str(round(np.log10(mstar_1e11), 2)) + r'$\log M_{\rm{vir}}$ = 11')
    ax2.axhline(dens_rh_1e10, color = 'red', label = r'$\log M_{\rm{star}} = $' + str(round(np.log10(mstar_1e10), 2))+ r'$\log M_{\rm{vir}}$ = 10')
    ax2.axhline(dens_rh_1e9, color = 'darkgreen', label = r'$\log M_{\rm{star}} = $' + str(round(np.log10(mstar_1e9), 2))+ r'$\log M_{\rm{vir}}$ = 9')
    ax2.axhline(dens_rh_1e8, color = 'purple', label = r'$\log M_{\rm{star}} = $' + str(round(np.log10(mstar_1e8), 2)) + r'$\log M_{\rm{vir}}$ = 8')
    ax2.axhline(dens_rh_1e6, color = 'orange', label = r'$\log M_{\rm{star}} = $' + str(round(np.log10(mstar_1e6), 2))+ r'$\log M_{\rm{vir}}$ = 6')
    ax2.axhline(dens_rh_1e4, color = 'cyan', label = r'$\log M_{\rm{star}} = $' + str(round(np.log10(mstar_1e4), 2))+ r'$\log M_{\rm{vir}}$ = 4')

    ax2.set_xlabel('Distance from center (kpc)')
    ax2.set_ylabel(r'Density $(<2rh)$ $[\rm{M_\odot/kpc^3}]$')
    ax2.set_xscale('log')
    ax2.set_yscale('log')
    ax2.legend(fontsize = 8)
    ...
    plt.tight_layout()
    plt.savefig(this_fof_plotppath + 'radial_density_distribution.png')
    plt.close()
    return None
...
def surv_comp(cmap = 'turbo_r'):
    ...
    pix = (psfof == 0) * (psmstar_max_ar > 10**8) & (psmstar_max_ar < 10**9.5)
