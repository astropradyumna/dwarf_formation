...
def plot_lg_virgo_some(ax, zorder = 300, alpha = 0.1, mec = 'black', vm = 'x', lgm = 'o', vms = 3, lgms = 3):
    '''
    this function plots all the Virgo and LG data
    We will only be using the some of the data in the main paper to avoid clutter
    '''
    ax.plot(np.log10(get_ms_from_mag(den_mg)), np.log10(den_re), marker = vm, mfc='None', mec = mec, label = 'Virgo Nucleated galaxies', 
            lw = 0, ms = vms, alpha = alpha, zorder = zorder)
    ax.plot(np.log10(mstr), np.log10(rh*1e3),marker=lgm,ms = lgms, mfc='None', mec = mec,ls='none', alpha = alpha, label = 'LG dwarfs', zorder = zorder)
